...
| Ошибка | Решение |
|--------|---------|
| `ModuleNotFoundError: No module named '...'` | Проверьте активацию виртуального окружения и установку зависимостей. |
| `KeyError: "['rd_regime', ...] not in index"` | Убедитесь, что используется актуальная версия `feature_pipeline.py` (должна генерировать эти фичи). |
| `XGBoost Library (libxgboost.dylib) could not be loaded` | Установите OpenMP: `brew install libomp`, затем переустановите XGBoost. |
| Мок‑сервер не отвечает (Connection refused) | Проверьте, что сервер запущен и слушает порт 3000. |
| Сигналы не отправляются, хотя предсказания есть | Проверьте пороги в `inference.py` (должны быть 0.75/0.25). |
