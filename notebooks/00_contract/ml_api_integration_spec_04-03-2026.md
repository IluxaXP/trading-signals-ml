...
## 2.1 Контур доступа (AUTH-02)

Публичный доступ к DS API выполняется через Basic Auth на уровне Nginx.

Профили доступа:
1. `operator` — полный доступ к публичному периметру (`/`, `/api/*`, `/ws`), включая DS API.
2. `dataset` — доступ только к `GET /api/ml/ds/*`.

Матрица доступа:
1. `GET /api/ml/ds/training-dataset`:
   - `operator`: доступ разрешен
   - `dataset`: доступ разрешен
2. `GET /api/ml/ds/feature-windows`:
   - `operator`: доступ разрешен
   - `dataset`: доступ разрешен
3. `GET /api/ml/export-dataset`:
   - `operator`: доступ разрешен
   - `dataset`: доступ запрещен
4. `POST /api/signals/ingest`:
   - `operator`: доступ разрешен
   - `dataset`: доступ запрещен (пока недоступно для пользователя `dataset`)
...
Примечание:
1. Пустой ответ `[]` для `format=json` является валидным результатом, если по фильтрам нет подходящих строк.

Пример запроса (dataset user):
```bash
curl -u 'dataset:<PASSWORD>' \
  'https://<YOUR_DOMAIN>/api/ml/ds/training-dataset?format=json&symbols=BTCUSDT'
```
...
```bash
curl -u 'dataset:<PASSWORD>' \
  'https://<YOUR_DOMAIN>/api/ml/ds/feature-windows?readyOnly=true'
```
...
Для пользователя `dataset` endpoint недоступен (доступ только к `GET /api/ml/ds/*`).
...
1. Legacy endpoint `GET /api/ml/export-dataset` остается доступным только для full-access пользователей (например, `operator`).
...
5. Версия `2.2.0` фиксирует AUTH-02: пользователь `dataset` ограничен только DS API (`GET /api/ml/ds/*`).
